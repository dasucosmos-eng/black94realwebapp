...
/* ── Image compression with filter + Firebase Storage upload ── */
function compressAndUploadWithFilter(
  file: File,
  filterCss: string,
  maxDim = 1200,
  quality = 0.82,
  ...
  if (file.type === 'image/gif') {
    return compressAndUpload(file, 'posts')
  }
        ...
        canvas.toBlob(
          async (blob) => {
            ...
            try {
              ...
              const url = await uploadToStorage(compressed, 'posts')
              resolve(url)
            } catch (err) {
              reject(err)
            }
...
export function ComposeDialog({ open, onClose }: ComposeDialogProps) {
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store raw preview and the file for later upload with selected filter
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (imagePreview && pendingFileRef.current) {
        mediaUrls = await compressAndUploadWithFilter(pendingFileRef.current, selectedFilter.css)
      }
